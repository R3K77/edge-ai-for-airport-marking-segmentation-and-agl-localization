...
def save_map_png(detections_csv: Path, traj_csv: Path, out_png: Path, max_points: int = 250000) -> None:
    ...
    traj = pd.read_csv(traj_csv) if traj_csv.exists() and traj_csv.stat().st_size > 0 else pd.DataFrame()
    if len(det) > max_points:
        det = det.sample(max_points, random_state=42)
    plt.figure(figsize=(12, 10))
    if not traj.empty and {"x_m", "y_m"}.issubset(traj.columns):
        plt.plot(traj["x_m"], traj["y_m"], linewidth=1.0, label="trajektoria")
    if {"x_m", "y_m", "cls_name"}.issubset(det.columns):
        for cls_name, g in det.groupby("cls_name"):
            plt.scatter(g["x_m"], g["y_m"], s=4, alpha=0.6, label=str(cls_name))
    plt.axis("equal")
    plt.grid(True, linewidth=0.3)
    plt.xlabel("x lokalne [m]")
    plt.ylabel("y lokalne [m]")
    plt.title("Detekcje / segmentacja w układzie lokalnym")
    plt.legend(loc="best")
    plt.tight_layout()
