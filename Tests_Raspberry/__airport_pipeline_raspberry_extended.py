...
"""
Zintegrowany pipeline dla Windows / Raspberry Pi:
...
Przykład:
python3 airport_pipeline_raspberry_extended.py \
    ...
    try:
        ...
        if not args.hrnet_model:
            args.hrnet_model = filedialog.askopenfilename(
                title="Wybierz model HRNet-Lite-Point (.onnx)",
                filetypes=[("Modele ONNX", "*.onnx *.ONNX"), ("Wszystkie pliki", "*.*")],
            )
            if not args.hrnet_model:
                raise RuntimeError("Nie wybrano modelu HRNet-Lite-Point.")

        if not args.linknet_model:
            args.linknet_model = filedialog.askopenfilename(
                title="Wybierz model LinkNet + MobileNetV2 (.onnx)",
                filetypes=[("Modele ONNX", "*.onnx *.ONNX"), ("Wszystkie pliki", "*.*")],
            )
            if not args.linknet_model:
                raise RuntimeError("Nie wybrano modelu LinkNet + MobileNetV2.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pełny pipeline wideo + GPS + dwa modele ONNX. W Windows bez argumentów otwiera okna wyboru plików.")
    parser.add_argument("--video", default=None, help="Ścieżka do pliku MP4 z osadzonym GPS. Gdy puste, pojawi się okno wyboru.")
    parser.add_argument("--hrnet_model", default=None, help="Ścieżka do modelu HRNet-Lite-Point w ONNX. Gdy puste, pojawi się okno wyboru.")
    parser.add_argument("--linknet_model", default=None, help="Ścieżka do modelu LinkNet + MobileNetV2 w ONNX. Gdy puste, pojawi się okno wyboru.")
    parser.add_argument("--out_dir", default=None, help="Folder wyjściowy. Gdy puste, pojawi się okno wyboru.")
...
def main() -> int:
    ...
    extended_metrics["hrnet_model_path"] = str(hrnet_model)
    extended_metrics["linknet_model_path"] = str(linknet_model)
    extended_metrics["out_dir"] = str(out_dir)
    extended_metrics["video_file_size_mb"] = video_path.stat().st_size / (1024 * 1024)
    extended_metrics["hrnet_model_size_mb"] = hrnet_model.stat().st_size / (1024 * 1024)
    extended_metrics["linknet_model_size_mb"] = linknet_model.stat().st_size / (1024 * 1024)
    extended_metrics["opencv_version"] = cv2.__version__
    extended_metrics["onnxruntime_version"] = ort.__version__
    ...
    extended_metrics["onnx_providers_hrnet"] = str(hrnet_runner.session.get_providers())
    extended_metrics["onnx_providers_linknet"] = str(linknet_runner.session.get_providers())
    extended_metrics["onnx_intra_threads"] = int(args.onnx_intra_threads)
    extended_metrics["onnx_inter_threads"] = int(args.onnx_inter_threads)
